...
async function loadTournament() {
  ...
  // Initialize handicap mode from tournament
  if (store.currentTournament) {
    editHandicapMode.value = store.currentTournament.handicap_mode || 'gross'
  }
}

function openRoundSettings() {
  editHandicapMode.value = store.currentTournament?.handicap_mode || 'gross'
  showRoundSettings.value = true
}

async function saveRoundSettings() {
  try {
    await fetch(`/api/tournaments/${tournamentId.value}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ handicap_mode: editHandicapMode.value })
    })
    await loadTournament()
    showRoundSettings.value = false
  } catch (err) {
...
<template>
  <div class="min-h-screen pb-40">
    ...
    <div class="sticky top-0 z-10 bg-dark/95 backdrop-blur-sm border-b border-gray-800 p-4">
      <div class="flex items-center justify-between mb-3">
        ...
        <div class="flex items-center gap-2">
          <button @click="openRoundSettings" class="text-gray-400 hover:text-white p-1" title="Round Settings">
    ...
    <div v-if="showRoundSettings" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div class="card max-w-sm w-full animate-slide-up">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-xl font-bold">Round Settings</h3>
          <button @click="showRoundSettings = false" class="text-gray-400 hover:text-white">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        ...
        <button @click="saveRoundSettings" class="w-full py-3 bg-golf-green rounded-xl font-semibold">
          Save Changes
        </button>
      </div>
    </div>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-dark border-t border-gray-800 p-4">
      <div class="flex justify-around max-w-md mx-auto">
        ...
        <button
          @click="router.push(`/tournament/${tournamentId}/quick`)"
          class="flex flex-col items-center text-gray-400"
        >
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
          </svg>
          <span class="text-xs mt-1">Quick</span>
        </button>
        <button
